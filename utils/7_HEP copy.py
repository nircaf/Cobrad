...
def _interpolate_nans_1d(arr):
    """
    Linearly interpolate NaNs in a 1D array. If all values are NaN, returns zeros.
...
def _ensure_finite_1d(arr):
    """
    Ensure a 1D array is finite (no NaN/Inf) by interpolating and filling.
    - Replace non-finite values with NaN
    - Interpolate internal NaNs
    - Fill any remaining NaNs/Infs with the mean (or 0 if mean is NaN)
    """
    arr = np.asarray(arr, dtype=float)
...
from scipy.stats import linregress
def _plot_metric_vs_hrv(t, arrs, labels, save_plot, edf_name, save_dir, is_streamlit=False, hue=None):
    """
    ...
    if len(arrs) == 2:
        ...
        # If a hue array was provided, attach it as a dedicated column (internal name)
        if hue is not None:
            # Ensure hue length matches rows (timepoints)
            try:
                if len(hue) == len(df_pair):
                    df_pair['__hue__'] = hue
                else:
                    # If hue doesn't match length, ignore it with a warning
                    print("Warning: provided `hue` length does not match data length; ignoring hue.")
            except Exception:
                print("Warning: invalid `hue` provided; ignoring hue.")

        g = sns.PairGrid(df_pair, diag_sharey=False)
        # Compute all unique pairwise p-values for FDR correction
        # exclude internal hue column from pairwise stats
        ...
        def scatter_with_stats(x, y, hue=None, **kwargs):
            ...
            # Find which pair this is (only among the plotted var_names)
            i, j = None, None
            ...
            x_clean = np.array(x)[mask]
            y_clean = np.array(y)[mask]
            # Handle hue if provided (mask it to match x/y)
            hue_clean = None
            if hue is not None:
                try:
        ...
        # If we attached a hue column, forward it to the scatter function
...
def compute_brain_heart_coupling(edf_results, key, motor_symptoms=None, bool_plots=False, save_plot=False, edf_pickle_name="",step_sec=5):
    ...
    # data_all, meta = slice_stratified_windows(raw, n_bins=6, window_s=600.0, windows_per_bin=3, seed=42)
    # accumulate results per band across outer windows
    band_to_dfs = {band: [] for band in power_bands}
    # run over each window in data_all
    for window_data in data_all:
        data_len = window_data.shape[1]
        ...
        # Sanitize ECG: replace non-finite and interpolate
        ...
        # Ensure finite after filtering as well (safety)
        ...
        # Process ECG signal (with robust fallback)
        try:
            signals, info = nk.ecg_process(ecg_signal_filt, sampling_rate=sfreq)
        except Exception as e:
            print(f"Warning: nk.ecg_process failed with error: {e}. Falling back to nk.ecg_clean.")
        ...
        # Remove from ecg_clean and from window_data where signals['ECG_Quality'] <= 0.5
        ...
        data = window_data[[ch_names.index(ch) for ch in eeg_channels if ch in ch_names]]
        ...
        for band in power_bands:
            ...
            # Add z-scored columns (now just copy original arrays, no zscore)
            results_df['Vagal_SD1_zscore'] = cvi_arr_band
            results_df['Sympathetic_SD2_zscore'] = csi_arr_band
            results_df['Efficiency_zscore'] = eff_arr
            results_df['Clustering_zscore'] = clu_arr
            results_df['Modularity_zscore'] = mod_arr
            results_df['Assortativity_zscore'] = ass_arr

            results_df.attrs['mutual_info'] = mi_results
            results_df_dict[band] = results_df
            band_to_dfs[band].append(results_df)

    # Compute average DataFrame per band across outer windows
    results_df_dict_avg = {}
    for band, dfs in band_to_dfs.items():
        if len(dfs) == 0:
            results_df_dict_avg[band] = pd.DataFrame()
            continue
        # Union of all columns across DFS
        all_columns = set()
        for df in dfs:
            all_columns.update(df.columns)
        all_columns = list(all_columns)
        # Align columns and rows (pad to max length)
        ...
        # Stack and compute nanmean with threshold >= half of the DFS
...
def process_all_patients(edf_results, step_sec=5):
    ...
    for patient_id, dfs in patient_results.items():
        ...
        plot_patient_band_means(            patient_id, bands=power_bands.keys(),
    ...
    plot_all_patients_band_means(bands=power_bands.keys(), step_sec=step_sec, temps_dir="temps_EDF_HEP", save_dir="figures_HEP/compute_brain_heart_coupling")
